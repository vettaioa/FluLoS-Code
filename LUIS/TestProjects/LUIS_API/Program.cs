    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            if(File.Exists(CREDENTIALS_PATH))
            {
                ...
                LuisJsonResponse parsed = JsonSerializer.Deserialize<LuisJsonResponse>(data);

                Type t = parsed.GetType(); // Where obj is object whose properties you need.
                PropertyInfo[] pi = t.GetProperties();
                foreach (PropertyInfo p in pi)
                {
                    System.Console.WriteLine(p.Name + " : " + p.GetValue(parsed));
                }
            }
            else
            {
